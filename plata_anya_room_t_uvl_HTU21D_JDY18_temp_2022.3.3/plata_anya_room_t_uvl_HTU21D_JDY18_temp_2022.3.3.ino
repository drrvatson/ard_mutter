...
#include "GyverPower.h"     //подгружаем библиотеку ввода в режим сна
...
void setup() {
  ...
  power.autoCalibrate(); // автоматическая калибровка режима сна
  power.setSleepMode(POWERDOWN_SLEEP);//режим сна 
...
if (nBlCtd >= 4) {                               //
  ...
  delay(1000);
  power.sleepDelay(300000);               // спим 1.5 секунды
}
